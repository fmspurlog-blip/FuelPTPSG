(async()=>{
  ...
  const pageVersion=params.get('v')||'78.6';
  const q='78.6-chart-layout-'+String(pageVersion).replace(/[^a-zA-Z0-9._-]/g,'');
  ...
  // Removed v783-ops-hotfix: it injects a second absolute-position legend outside panels.
  setTimeout(()=>safeLoad('v784-responsive-performance.js?v='+q),700);
  setTimeout(()=>safeLoad('v785-chart-calendar.js?v='+q),900);
  setTimeout(()=>safeLoad('v786-chart-layout.js?v='+q),1100);
