(async()=>{
  ...
  function timeIso(v){
    ...
    if(typeof v==='number'&&Number.isFinite(v)){
      const frac=((v%1)+1)%1;const total=Math.round(frac*86400)%86400;
      const h=Math.floor(total/3600),m=Math.floor((total%3600)/60);
      return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`;
    }
    const s=String(v).trim();
    if(/^\d*\.\d+$/.test(s)){const n=Number(s);if(Number.isFinite(n)&&n>=0&&n<1)return timeIso(n)}
    ...
    const m=s.match(/(\d{1,2}):(\d{2})/);return m?`${String(Number(m[1])).padStart(2,'0')}:${m[2]}`:s;
  }
