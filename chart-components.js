...
const plugin={
 ...
 beforeInit(chart){const id=chartID(chart),opts=chart.config.options;
  ...
  // Render daily bars immediately after dashboard navigation; animation could leave
  // screenshots showing axes without bars even when chart geometry was valid.
  ...
  if(id==='dailyChart'){opts.animation=false;opts.plugins=opts.plugins||{};const labels=opts.plugins.datalabels||{};opts.plugins.datalabels={...labels,display:context=>{const meta=context.chart.getDatasetMeta(context.datasetIndex),bars=meta?.data||[],bar=bars[context.dataIndex];if(!bar)return false;const prev=bars[context.dataIndex-1],next=bars[context.dataIndex+1];const width=30;return (!prev||Math.abs(bar.x-prev.x)>=width)&&(!next||Math.abs(next.x-bar.x)>=width)}}}
 },
