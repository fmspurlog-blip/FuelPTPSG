...
function paintBrand(){
  ...
  const ok=box.querySelector('.v783-brand');
  if(!ok) box.innerHTML=`<div class="v783-brand">${fuelGun}<div class="v783-brand-text">REFUELING<br>CONTROL</div></div>`;
}

function applyV783(){
  paintBrand();
  const icons=document.querySelectorAll('.unit-type-panel .unit-icon');
  if(icons[0]&&!icons[0].querySelector('svg')) icons[0].innerHTML=excavator;
  if(icons[1]&&!icons[1].querySelector('svg')) icons[1].innerHTML=gears;
...
setTimeout(applyV783,300);setTimeout(applyV783,1500);setTimeout(applyV783,3500);

const brandBox=document.querySelector('.logo-box');
if(brandBox){
  const observer=new MutationObserver(()=>{
    if(!brandBox.querySelector('.v783-brand')) queueMicrotask(paintBrand);
  });
