(()=>{
  ...
  const getStock=()=>typeof stock!=='undefined'?stock:null;
  const getRecon=()=>typeof reconData!=='undefined'?reconData:null;
  ...
  function syncStockTotal(){
    try{
      ...
      if(s&&calculated>0) s.total=calculated;
      const totalEl=document.getElementById('stockTotal');
      if(totalEl) totalEl.textContent=fmt(s?.total||calculated);
  ...
  function reportRows(){
    const d=Array.isArray(getState()?.filtered)?getState().filtered:[];
    return d.map(r=>({Date:r.Date||'',Time:r.Time||'',Shift:r.Shift||'',Unit_Code:r.Unit_Code||'',Category:r.Category||'',Unit_Type:r.Unit_Type||'',Fuel_Truck:r.Fuel_Truck||'',Fuel_Liter:Number(r.Fuel_Liter)||0,Actual_LHM:r.Actual_LHM??'',Standard_LHM:r.Standard_LHM??'',Variance_Pct:r.Variance_Pct??'',Consumption_Status:r.Consumption_Status||''}));
  }
  ...
  function exportDashboardExcel(){
    try{
      ...
      const summary=[['FUEL MANAGEMENT SYSTEM - DASHBOARD EXPORT',''],['PT PRIMA SARANA GEMILANG SITE ABM',''],['Period From',from],['Period To',to],['Shift',document.getElementById('shiftFilter')?.value||'All Shift'],['Category',document.getElementById('categoryFilter')?.value||'All Category'],['Fuel Truck',document.getElementById('truckFilter')?.value||'All Fuel Truck'],['Search Unit',document.getElementById('unitSearch')?.value||''],['Total Fuel Issued (Liter)',total],['Average / Day (Liter)',days?total/days:0],['Active Unit',units],['Over Consumption (Liter)',over],['Efficient (Liter)',efficient],['Physical Stock (Liter)',Number(s?.total)||0],['Stock Snapshot Date',s?.snapshotDate||''],['Stock Snapshot Time',s?.snapshotTime||'']];
      const wsSummary=XLSX.utils.aoa_to_sheet(summary);wsSummary['!cols']=[{wch:34},{wch:24}];XLSX.utils.book_append_sheet(wb,wsSummary,'Dashboard Summary');
      const wsUsage=XLSX.utils.json_to_sheet(rows);wsUsage['!cols']=[{wch:12},{wch:10},{wch:10},{wch:16},{wch:18},{wch:18},{wch:18},{wch:14},{wch:14},{wch:14},{wch:14},{wch:22}];XLSX.utils.book_append_sheet(wb,wsUsage,'Fuel Usage');
      const stockRows=[];Object.entries(s?.fuelStorage||{}).forEach(([unit,value])=>stockRows.push({Stock_Type:'STATIC TANK',Unit:unit,Liter:Number(value)||0,Snapshot_Date:s?.snapshotDate||'',Snapshot_Time:s?.snapshotTime||''}));Object.entries(s?.fuelTruck||{}).forEach(([unit,value])=>stockRows.push({Stock_Type:'FUEL TRUCK',Unit:unit,Liter:Number(value)||0,Snapshot_Date:s?.snapshotDate||'',Snapshot_Time:s?.snapshotTime||''}));XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(stockRows),'Fuel Stock');
  ...
  function installExcelOnlyReport(){
    ...
    if(!card){const reconBtn=reports.querySelector('[data-report="recon"]');card=reconBtn?.closest('.report-card');}
    if(!card)return;
    ...
    const btn=document.getElementById('exportDashboardExcel');if(btn&&!btn.dataset.bound){btn.dataset.bound='1';btn.addEventListener('click',exportDashboardExcel);}
  }

  function afterRender(){setVersion();syncStockTotal();tidyTitles();polishDonuts();installExcelOnlyReport();resizeVisibleCharts();}
  afterRender();setTimeout(afterRender,350);setTimeout(afterRender,1100);
  ['dateFrom','dateTo','shiftFilter','categoryFilter','truckFilter'].forEach(id=>document.getElementById(id)?.addEventListener('change',()=>setTimeout(afterRender,40)));
  ...
  window.addEventListener('hashchange',()=>setTimeout(afterRender,80));window.addEventListener('resize',resizeVisibleCharts);
})();