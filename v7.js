        ...
        if(name==='daily'){
          ...
          if(c.options.plugins.datalabels){
            ...
            c.options.plugins.datalabels.formatter=(v,ctx)=>{
              ...
              if(count>20 && ctx.dataIndex%2===1)return '';
              return fmt(v);
            };
          }
          c.options.scales.x.ticks.maxTicksLimit=11;
        }
        if(name==='shift'){
          c.data.datasets.forEach(ds=>ds.borderWidth=3);
        }
        if(name==='cat'){
          if(c.options.plugins.datalabels)c.options.plugins.datalabels.font={size:8,weight:'700'};
        }
        if(name==='truck'||name==='status'){
          if(c.options.plugins.legend)c.options.plugins.legend.position='right';
        }
        c.update('none');
      }
    }catch(err){console.warn('V7 chart polish',err)}
  };

  const observer=new MutationObserver(()=>{cleanShiftLegend();styleRecon();});
  const legend=document.getElementById('shiftLegend');if(legend)observer.observe(legend,{childList:true,subtree:true});
  const recon=document.getElementById('kpiReconStatus');if(recon)observer.observe(recon,{childList:true,characterData:true,subtree:true});

  cleanShiftLegend();styleRecon();addVersion();polishCharts();
  setInterval(()=>{cleanShiftLegend();styleRecon();polishCharts();},1200);
})();