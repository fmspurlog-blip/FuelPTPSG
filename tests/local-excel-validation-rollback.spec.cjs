/* Offline regression: failed validation or rendering must not replace existing dashboard data. */
'use strict';
...
async function scenario(failure){
  ...
  vm.runInNewContext(source,{document,state,window:{FUEL_SAFE_UPLOAD_CORE:{validate:()=>{validations++;if(failure==='validate')throw Error('validator rejected')}}},XLSX:{read:()=>({SheetNames:['Fuel_Usage_Clean'],Sheets:{Fuel_Usage_Clean:{}}}),utils:{sheet_to_json:()=>[{Date:'2026-09-02',Unit_Code:'NEW',Fuel_Liter:12}]}},normalize:rows=>rows,initFilters:()=>{filterCalls++;if(failure==='filters'&&filterCalls===1)throw Error('filters failed')},applyFilters:()=>{renders++;if(failure==='render'&&renders===1)throw Error('render failed')},alert:message=>alerts.push(message),Date,Number,String,Object,Set,Error});
  let stopped=false;
  ...
  assert.equal(validations,1,'validator must run exactly once');
...
(async()=>{
  ...
  console.log('PASS: validator, filter and render failures preserve old data, restore UI and clear input (offline).');
